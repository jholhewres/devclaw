...
func (s *Server) handleSetupFinalize(w http.ResponseWriter, r *http.Request) {
	...
	if s.onVaultInit != nil && setup.APIKey != "" {
		...
		if err := s.onVaultInit(vaultPassword, secrets); err != nil {
			...
			// Fallback: write to .env so the system still works.
			...
			s.logger.Info("vault created and API key stored securely")
			// Write both API key and vault password to .env.
			// The API key env var is needed because config.yaml references ${GOCLAW_API_KEY}.
			// The vault password enables non-interactive unlock (PM2, Docker, systemd).
			envContent := "# GoClaw — generated by setup wizard\n" +
				"GOCLAW_API_KEY=" + setup.APIKey + "\n" +
				"GOCLAW_VAULT_PASSWORD=" + vaultPassword + "\n"
			if err := os.WriteFile(".env", []byte(envContent), 0o600); err != nil {
				s.logger.Error("failed to write .env", "error", err)
			}
		}
...
// First tries from embedded defaults, then downloads from goclaw-skills on GitHub.
func (s *Server) installSetupSkills(selected []string) {
	const skillsDir = "./skills"
	const ghBase = "https://raw.githubusercontent.com/jholhewres/goclaw-skills/main/skills"
...
// First tries to fetch the catalog from goclaw-skills on GitHub; falls back to embedded defaults.
func (s *Server) handleSetupSkills(w http.ResponseWriter, r *http.Request) {
	...
	// Try fetching the catalog from the goclaw-skills repository.
...
// skillsCatalogEntry represents a single skill in the goclaw-skills index.yaml.
...
// skillsCatalogFile represents the top-level structure of goclaw-skills index.yaml.
type skillsCatalogFile struct {
	Skills map[string]skillsCatalogEntry `yaml:"skills"`
}

// fetchSkillsCatalogFromGitHub fetches and parses the goclaw-skills index.yaml from GitHub.
func fetchSkillsCatalogFromGitHub(logger interface{ Debug(string, ...any) }) []map[string]any {
	const catalogURL = "https://raw.githubusercontent.com/jholhewres/goclaw-skills/main/index.yaml"
