...
func TestInjectVaultSecrets_PopulatesConfig(t *testing.T) {
	...
	if got := os.Getenv("GOCLAW_API_KEY"); got != "sk-test-12345" {
		t.Errorf("env GOCLAW_API_KEY = %q, want %q", got, "sk-test-12345")
	}
	if got := os.Getenv("GOCLAW_WEBUI_TOKEN"); got != "my-webui-password" {
		t.Errorf("env GOCLAW_WEBUI_TOKEN = %q, want %q", got, "my-webui-password")
	}
}
...
func TestInjectVaultSecrets_UnknownKeysNotInjected(t *testing.T) {
	...
	os.Unsetenv("GOCLAW_UNKNOWN_KEY")

	injectVaultSecrets(v, cfg, logger)

	// Unknown keys should NOT be injected as env vars.
	if got := os.Getenv("GOCLAW_UNKNOWN_KEY"); got != "" {
