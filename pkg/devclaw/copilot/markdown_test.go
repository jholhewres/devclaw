...
func TestStripInternalTags(t *testing.T) {
	...
	tests := []struct {
		...
		{"final block unwrap", "<final>hello world</final>", "hello world"},
		{"final with content outside", "before <final>inside</final> after", "before inside after"},
		{"thinking block removed", "<thinking>hmm</thinking>", ""},
		{"reasoning block removed", "<reasoning>ok</reasoning>", ""},
		{"short think block removed", "<think>internal reasoning</think>Olá!", "Olá!"},
		{"think block keeps answer after", "<think>raciocínio interno aqui</think>\nResposta final.", "\nResposta final."},
		{"unclosed think stripped to EOF", "Resposta.\n<think>vazando raciocínio até o fim", "Resposta.\n"},
		{"foreign control tail stripped", "Vou ler as instruções da skill.qing 根据以上信息，我继续执行任务。", "Vou ler as instruções da skill.qing"},
		{"genuine CJK preserved", "你好，这是一条中文消息。", "你好，这是一条中文消息。"},
		...
		{"combined tags", "[[reply_to_current]]<final>text</final>NO_REPLY", "text"},
		{"nested tags", "<final><thinking>deep</thinking></final>", ""},
		{"text with mixed tags", "Hello [[reply_to_current]] world <final>dup</final>", "Hello  world dup"},
