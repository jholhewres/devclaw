...
type claudeCodeSkill struct {
	...
	// sessions maps DevClaw session key → last Claude Code session ID,
...
func NewClaudeCodeSkill(provider ProjectProvider, apiKey, baseURL, defaultModelName string) Skill {
	...
	var budget float64
	if budgetStr := os.Getenv("DEVCLAW_CLAUDE_CODE_BUDGET"); budgetStr != "" {
	...
	timeoutMin := 18
	if v := os.Getenv("DEVCLAW_CLAUDE_CODE_TIMEOUT_MIN"); v != "" {
...
Send clear, detailed instructions. The task runs in the active project directory.`,
			Parameters: []ToolParameter{
				...
				{Name: "session_key", Type: "string", Description: "DevClaw session key (auto-provided by system)."},
...
func progressSenderFromCtx(ctx context.Context) func(context.Context, string) {
	...
	if ps, ok := ctx.Value("devclaw.progress_sender").(progressSenderFunc); ok {
		return ps
	}
	return nil
}
